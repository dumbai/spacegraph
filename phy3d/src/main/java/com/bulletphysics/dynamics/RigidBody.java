...
public class RigidBody extends CollisionObject {
    ...
    public RigidBody(RigidBodyConstructionInfo c) {
        ...
        interpolationLinearVelocity.set(0.0f, 0.0f, 0.0f);
        interpolationAngularVelocity.set(0.0f, 0.0f, 0.0f);
    ...
    public void setGravity(Vector3f acceleration) {
        if (inverseMass != 0.0f) {
            gravity.scale(1.0f / inverseMass, acceleration);
        }
    }
    ...
    public void setDamping(float lin_damping, float ang_damping) {
        linearDamping = MiscUtil.GEN_clamped(lin_damping, 0.0f, 1.0f);
        angularDamping = MiscUtil.GEN_clamped(ang_damping, 0.0f, 1.0f);
    }
    ...
    public void applyDamping(float timeStep) {
        ...
        //#else
        linearVelocity.scale((float) Math.pow(1.0f - linearDamping, timeStep));
        angularVelocity.scale((float) Math.pow(1.0f - angularDamping, timeStep));
        //#endif
    ...
    public void setMassProps(float mass, Vector3f inertia) {
        ...
        invInertiaLocal.set(inertia.x == 0f ? 0f : 1f / inertia.x,
                inertia.y == 0f ? 0f : 1f / inertia.y,
                inertia.z == 0f ? 0f : 1f / inertia.z);
    }
    ...
    public void clearForces() {
        totalForce.set(0.0f, 0.0f, 0.0f);
        totalTorque.set(0.0f, 0.0f, 0.0f);
    }
