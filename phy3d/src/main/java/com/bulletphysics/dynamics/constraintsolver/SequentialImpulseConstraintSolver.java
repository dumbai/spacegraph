...
public class SequentialImpulseConstraintSolver extends ConstraintSolver {
    ...
    private static void initSolverBody(SolverBody solverBody, CollisionObject collisionObject) {
    ...
    private static void resolveSplitPenetrationImpulseCacheFriendly(
        ...
        if (contactConstraint.penetration < solverInfo.splitImpulsePenetrationThreshold) {
            ...
            contactConstraint.appliedPushImpulse = Math.max(0.0f, sum);
    ...
    private static float resolveSingleCollisionCombinedCacheFriendly(
...

        {
            ...
            float sum = oldNormalImpulse + normalImpulse;
            contactConstraint.appliedImpulse = Math.max(0.0f, sum);
    ...
    private static float resolveSingleFrictionCacheFriendly(
        ...
        if (appliedNormalImpulse > 0.0f) //friction
        {
    ...
    public static float solveCombinedContactFriction(RigidBody body0, RigidBody body1, ManifoldPoint cp, ContactSolverInfo info, int iter, IDebugDraw debugDrawer) {
            ...
            int iteration;
            {
                for (iteration = 0; iteration < infoGlobal.numIterations; iteration++) {
                    ...
                    for (j = 0; j < numConstraints; j++) {
                        ...
                        RigidBody ra = constraint.getRigidBodyA();
                        if ((ra.getIslandTag() >= 0) && (ra.getCompanionId() >= 0)) {
                            tmpSolverBodyPool.get(ra.getCompanionId()).writebackVelocity();
                        }
                        RigidBody rb = constraint.getRigidBodyB();
                        if ((rb.getIslandTag() >= 0) && (rb.getCompanionId() >= 0)) {
                            tmpSolverBodyPool.get(rb.getCompanionId()).writebackVelocity();
                        }

                        constraint.solveConstraint(infoGlobal.timeStep);

                        if ((ra.getIslandTag() >= 0) && (ra.getCompanionId() >= 0)) {
                            tmpSolverBodyPool.get(ra.getCompanionId()).readVelocity();
                        }
                        if ((rb.getIslandTag() >= 0) && (rb.getCompanionId() >= 0)) {
                            tmpSolverBodyPool.get(rb.getCompanionId()).readVelocity();
                        }
                    }
