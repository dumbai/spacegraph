...
public class ScaledBvhTriangleMeshShape extends ConcaveShape {
	...
	private final Vector3f localScaling = new Vector3f();
	...
	public void processAllTriangles(TriangleCallback callback, Vector3f aabbMin, Vector3f aabbMax) {
		...
		invLocalScaling.set(1.0f / localScaling.x, 1.0f / localScaling.y, 1.0f / localScaling.z);

		Vector3f scaledAabbMin = new Vector3f();
		Vector3f scaledAabbMax = new Vector3f();

		// support negative scaling
		scaledAabbMin.x = localScaling.x >= 0.0f ? aabbMin.x * invLocalScaling.x : aabbMax.x * invLocalScaling.x;
		scaledAabbMin.y = localScaling.y >= 0.0f ? aabbMin.y * invLocalScaling.y : aabbMax.y * invLocalScaling.y;
		scaledAabbMin.z = localScaling.z >= 0.0f ? aabbMin.z * invLocalScaling.z : aabbMax.z * invLocalScaling.z;

		scaledAabbMax.x = localScaling.x <= 0.0f ? aabbMin.x * invLocalScaling.x : aabbMax.x * invLocalScaling.x;
		scaledAabbMax.y = localScaling.y <= 0.0f ? aabbMin.y * invLocalScaling.y : aabbMax.y * invLocalScaling.y;
		scaledAabbMax.z = localScaling.z <= 0.0f ? aabbMin.z * invLocalScaling.z : aabbMax.z * invLocalScaling.z;
	...
	public void getAabb(Transform trans, Vector3f aabbMin, Vector3f aabbMax) {
		...
		localAabbMin.x = (localScaling.x >= 0.0f) ? tmpLocalAabbMin.x : tmpLocalAabbMax.x;
		localAabbMin.y = (localScaling.y >= 0.0f) ? tmpLocalAabbMin.y : tmpLocalAabbMax.y;
		localAabbMin.z = (localScaling.z >= 0.0f) ? tmpLocalAabbMin.z : tmpLocalAabbMax.z;
		localAabbMax.x = (localScaling.x <= 0.0f) ? tmpLocalAabbMin.x : tmpLocalAabbMax.x;
		localAabbMax.y = (localScaling.y <= 0.0f) ? tmpLocalAabbMin.y : tmpLocalAabbMax.y;
		localAabbMax.z = (localScaling.z <= 0.0f) ? tmpLocalAabbMin.z : tmpLocalAabbMax.z;
	...
	private static class ScaledTriangleCallback extends TriangleCallback {
		...
		ScaledTriangleCallback(TriangleCallback originalCallback, Vector3f localScaling) {
			this.originalCallback = originalCallback;
			this.localScaling = localScaling;

			Arrays.setAll(newTriangle, i -> new Vector3f());
		}
