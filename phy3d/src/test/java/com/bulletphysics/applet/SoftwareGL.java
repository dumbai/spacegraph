...
package com.bulletphysics.applet;
...
public class SoftwareGL implements IGL {
	...
	public void glLight(int light, int pname, float[] params) {
		...
		switch (pname) {
			case GL_AMBIENT -> l.ambient.set(params);
			case GL_DIFFUSE -> l.diffuse.set(params);
			case GL_SPECULAR -> l.specular.set(params);
			case GL_POSITION -> {
	...
	public void glEnable(int cap) {
		...
		switch (cap) {
			case GL_LIGHTING -> gl.setLightingEnabled(true);
		}
	}
	...
	public void glDisable(int cap) {
		...
		switch (cap) {
			case GL_LIGHTING -> gl.setLightingEnabled(false);
		}
	}
	...
	private void setMatrix(Matrix4f mat) {
		switch (matrixMode) {
			case GL_MODELVIEW -> gl.setViewMatrix(mat);
			case GL_PROJECTION -> gl.setProjMatrix(mat);
	...
	private void mulMatrix(Matrix4f mat) {
		switch (matrixMode) {
			case GL_MODELVIEW -> gl.mulViewMatrix(mat);
			case GL_PROJECTION -> gl.mulProjMatrix(mat);
			default -> throw new IllegalStateException();
		}
	}
	...
	public void glBegin(int mode) {
		switch (mode) {
			case GL_LINES -> gl.begin(Graphics3D.LINES);
			case GL_TRIANGLES -> gl.begin(Graphics3D.TRIANGLES);
			case GL_QUADS -> gl.begin(Graphics3D.QUADS);
	...
	public void drawCube(float extent) {
	    ...
	    glBegin(GL_QUADS);
        glNormal3f(1.0f, 0.0f, 0.0f); glVertex3f(+extent,-extent,+extent); glVertex3f(+extent,-extent,-extent); glVertex3f(+extent,+extent,-extent); glVertex3f(+extent,+extent,+extent);
        glNormal3f(0.0f, 1.0f, 0.0f); glVertex3f(+extent,+extent,+extent); glVertex3f(+extent,+extent,-extent); glVertex3f(-extent,+extent,-extent); glVertex3f(-extent,+extent,+extent);
        glNormal3f(0.0f, 0.0f, 1.0f); glVertex3f(+extent,+extent,+extent); glVertex3f(-extent,+extent,+extent); glVertex3f(-extent,-extent,+extent); glVertex3f(+extent,-extent,+extent);
        glNormal3f(-1.0f, 0.0f, 0.0f); glVertex3f(-extent,-extent,+extent); glVertex3f(-extent,+extent,+extent); glVertex3f(-extent,+extent,-extent); glVertex3f(-extent,-extent,-extent);
        glNormal3f(0.0f,-1.0f, 0.0f); glVertex3f(-extent,-extent,+extent); glVertex3f(-extent,-extent,-extent); glVertex3f(+extent,-extent,-extent); glVertex3f(+extent,-extent,+extent);
        glNormal3f(0.0f, 0.0f,-1.0f); glVertex3f(-extent,-extent,-extent); glVertex3f(-extent,+extent,-extent); glVertex3f(+extent,+extent,-extent); glVertex3f(+extent,-extent,-extent);
		glEnd();
	...
	public void drawCylinder(float radius, float halfHeight, int upAxis) {
		...
		switch (upAxis) {
			...
			case 2 -> glTranslatef(0.0f, 0.0f, -halfHeight);
			default -> {
				assert (false);
			}
		}
		...
		cylinder.draw(gl, radius, radius, 2.0f * halfHeight, 8, 1);
